        ...
        protected ActionResult ResponseApi(object? result = null)
        {
            if (!ModelState.IsValid)
            {
                for (var i = 0; i < ModelState.Count; i++)
                    _notifications.Handle(
                        new DomainNotification(ModelState.Keys.ToList()[i],
                            ModelState.Values.ToList()[i].Errors.FirstOrDefault().ErrorMessage),
            ...
            return BadRequest(new Result
            {
                ...
                Errors = _notifications.GetNotifications()
                    .Select(n => new MessageError { Key = n.Key, Message = n.Value }).ToArray()
            });
        }
    }
}